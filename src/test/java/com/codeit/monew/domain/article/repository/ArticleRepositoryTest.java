...
class ArticleRepositoryTest {
    ...
    @Test
    @DisplayName("날짜 범위 조건으로 뉴스 기사 목록 필터링")
    ...
    void searchArticles_fetchNextPageByPublishDateCursor() {
        ...
        ArticleSearchRequest secondRequest = new ArticleSearchRequest(
                ...
                LocalDateTime.parse(firstPage.nextAfter()),
                2,
                UUID.randomUUID()
        );

        // when
        CursorPageResponse<Article> secondPage = articleRepository.searchArticles(secondRequest);
    ...
    void findByIdAndDeletedAtIsNull_success() {
        ...
        // when
        Optional<Article> foundArticle = articleRepository.findByIdAndDeletedAtIsNull(savedArticle.getId());
